...
namespace ChuongCustom
{
    ...
    public class ScreenManager : Singleton<ScreenManager>
    {
        ...
        private void Start()
        {
            _listScreens = GetComponentsInChildren<BasePopup>().ToList();

            if (_listScreens.Count > 0)
            {
                _currentScreen = _listScreens[^1];
            }
        ...
        public void OpenScreen(ScreenType screenType)
        {
            ...
            _overlay.gameObject.SetActive(screen.GetID() != ScreenType.HomeScreen && screen.GetID() != ScreenType.HUD);
